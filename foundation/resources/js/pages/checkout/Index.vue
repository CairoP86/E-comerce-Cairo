...
<template><Head title="Revisa y confirma tu pedido"><meta name="robots" content="noindex, nofollow"/></Head><StoreLayout>
    ...
    <form class="st-checkout-grid" novalidate @submit.prevent="submit" :aria-busy="form.processing">
        ...
        <aside id="checkout-review" class="st-checkout-review" tabindex="-1"><p class="st-overline">3. REVISIÓN</p><OrderSummary :items="review.items" :currency="review.currency" :subtotal="review.subtotal_minor" :shipping="review.shipping" :total="review.total_minor" :tax="review.tax"/><p>Revisa tus datos y los productos antes de confirmar.</p><button class="st-button" type="submit" :disabled="form.processing || quoting || !review.shipping">{{ form.processing ? 'Creando pedido…' : 'Confirmar pedido' }}</button><p class="st-cart-help" role="status">El pedido quedará pendiente de pago. El total incluye el envío y no cambiará después de confirmarlo.</p><Link href="/cart" class="st-text-link">Volver al carrito</Link></aside>
    </form>
