...
<template>
    <StoreSeo :seo="seo"/><StoreLayout>
        ...
        <div v-else class="st-cart-grid" :aria-busy="busy">
            ...
            <aside class="st-cart-summary" aria-labelledby="cart-summary-title"><p class="st-overline">TU SELECCIÓN, EN CLARO</p><h2 id="cart-summary-title">Resumen del carrito</h2><dl><div><dt>{{ cart.has_unavailable ? 'Subtotal de productos incluidos' : 'Subtotal de productos' }}</dt><dd>{{ money(cart.subtotal_minor, cart.currency!) }}</dd></div><div class="st-cart-total"><dt>Total provisional</dt><dd v-if="cart.total_minor !== null">{{ money(cart.total_minor, cart.currency!) }}</dd><dd v-else>Por revisar</dd></div></dl><p v-if="cart.has_unavailable" class="st-cart-unavailable" role="status">Hay productos que cambiaron. Retíralos para volver a calcular el total provisional.</p><p class="st-cart-summary-note">{{ cart.currency }} · Importes de productos únicamente. La entrega y los impuestos definitivos, cuando correspondan, se tratarán al habilitar la compra.</p><div class="st-cart-next"><h3>Un paso a la vez.</h3><p>Puedes guardar y ajustar esta selección. El checkout y los pagos todavía no están habilitados.</p></div><Link href="/catalog" class="st-button">Seguir explorando ↗</Link><p class="st-cart-help">Los precios se actualizan al consultar tu carrito. Agregar un producto no reserva stock ni confirma una compra.</p></aside>
        </div>
    </StoreLayout>
</template>
