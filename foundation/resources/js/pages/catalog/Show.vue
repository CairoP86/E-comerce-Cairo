...
<template>
    <StoreSeo :seo="seo"/><StoreLayout>
        ...
        <div class="st-detail">
            ...
            <section class="st-product-summary"><Link class="st-overline st-brand-link" :href="catalogUrl({ brand: product.brand.slug })">{{ product.brand.name }} ↗</Link><h1>{{ product.name }}</h1><p class="st-sku">SKU {{ product.sku }} <span>·</span> {{ product.is_demo ? 'Demostración' : 'Publicado en catálogo' }}</p><p class="st-summary-description">{{ product.short_description }}</p><div class="st-detail-price"><del v-if="product.previous_price_minor">{{ money(product.previous_price_minor, product.currency) }}</del><strong>{{ money(product.price_minor, product.currency) }}</strong><span>{{ product.currency }} · {{ product.is_demo ? 'Precio ilustrativo' : 'Precio publicado' }}</span></div><StockStatus :availability="availability[product.slug]" size="detail"/><AddToCart :slug="product.slug" :availability="availability[product.slug]"/><div class="st-summary-links"><a href="#warranty">Información de garantía ↗</a><Link :href="catalogUrl({ category: product.category.slug })">Más en {{ product.category.name }} ↗</Link></div></section>
        </div>
