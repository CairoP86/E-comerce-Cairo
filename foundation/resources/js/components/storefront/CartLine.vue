...
<template>
    <article class="st-cart-line" :class="{ 'is-unavailable': !line.available }">
        ...
        <div class="st-cart-line-content">
            ...
            <p v-else class="st-cart-unavailable">{{ line.reason === 'currency_changed' ? 'La moneda de este producto cambió. Retíralo para continuar.' : 'Este producto ya no está publicado en el catálogo. Retíralo para continuar.' }} No se incluye en el subtotal.</p>
            <form v-if="line.available" class="st-cart-quantity-form" @submit.prevent="emit('update', quantity)">
