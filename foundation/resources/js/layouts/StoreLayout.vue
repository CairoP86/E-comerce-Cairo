    ...
    <div class="storefront">
        ...
        <header class="st-header" @keydown.esc="closeMenu">
            ...
            <nav id="store-navigation" class="st-container st-nav" :class="{ 'is-open': menu }" aria-label="Navegación principal">
                ...
                <Link href="/catalog">Explorar catálogo <span aria-hidden="true">↗</span></Link><Link href="/catalog?featured=1">Destacados</Link><Link href="/catalog?sort=newest">Novedades</Link><a href="/#brands">Marcas</a><Link href="/metodos-de-pago">Métodos de Pago</Link><Link href="/venta-corporativa">Venta Corporativa</Link><span class="st-nav-region">{{ page.props.identity.region }}</span>
            </nav>
        </header>
        <main id="main-content" tabindex="-1" class="st-container"><slot /></main>
        <footer class="st-footer"><div class="st-container"><div class="st-footer-grid"><div class="st-footer-brand"><Link href="/" class="st-logo"><span class="st-logo-mark" aria-hidden="true">{{ page.props.identity.mark }}</span>{{ page.props.identity.name }}</Link><p>{{ page.props.identity.tagline }}</p><p>Un espacio para explorar, conocer y elegir tu próximo equipo.</p></div><nav aria-label="Explorar"><h2>Explora</h2><Link href="/catalog">Todo el catálogo</Link><Link href="/catalog?featured=1">Destacados</Link><Link href="/catalog?sort=newest">Novedades</Link><a href="/#brands">Marcas</a></nav><nav aria-label="Cuenta"><h2>Tu espacio</h2><Link href="/login">Iniciar sesión</Link><Link href="/register">Crear cuenta</Link><Link href="/account">Mi cuenta</Link></nav><div><h2>Información del catálogo</h2><p>Las condiciones de venta, entrega y atención se publicarán antes de la apertura comercial.</p></div></div><div class="st-footer-bottom"><span>{{ page.props.identity.name }} · {{ page.props.identity.region }}</span><a href="#main-content">Volver arriba ↑</a></div></div></footer>
    </div>
</template>
